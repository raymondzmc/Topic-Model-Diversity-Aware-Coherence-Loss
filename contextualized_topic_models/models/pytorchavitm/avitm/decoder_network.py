...
from contextualized_topic_models.models.pytorchavitm.avitm.inference_network import InferenceNetwork
...
class DecoderNetwork(nn.Module):
    ...
    def __init__(self, input_size, n_components=10, model_type='prodLDA',
                 ...
                 learn_priors=True, topic_prior_mean=0.0, topic_prior_variance=None, device='cpu'):
        """
        ...
        self.inf_net = InferenceNetwork(
            input_size, n_components, hidden_sizes, activation).to(self.device)
        ...
        self.prior_variance = torch.tensor(
            [topic_prior_variance] * n_components).to(self.device)
        if self.learn_priors:
            self.prior_variance = nn.Parameter(self.prior_variance)

        self.beta = torch.Tensor(n_components, input_size).to(self.device)
